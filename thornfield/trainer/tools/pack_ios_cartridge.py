...
Usage:
    python3 thornfield/trainer/tools/pack_ios_cartridge.py amber_cipher

Input:  thornfield/trainer/cases/<case_id>/  (spec.json, tokens.json, graph.json)
Output: thornfield/ios/Cartridge/Bundles/<Title>.cartridge/
...
def pack_ios_cartridge(case_id: str, repo_root: Path) -> Path:
    cases_dir = repo_root / "thornfield" / "trainer" / "cases" / case_id
    ...
    out_dir = repo_root / "thornfield" / "ios" / "Cartridge" / "Bundles" / f"{bundle_name}.cartridge"
