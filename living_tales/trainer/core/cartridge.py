...
class CartridgeSpec:
    ...
    def load(cls, spec_path: str) -> "CartridgeSpec":
        ...
        mode = infer_game_mode(
            (item["attractor_weights"] for item in tokens_data),
            spec.get("mode"),
        )
        lower_bound, upper_bound = default_dimension_bounds(mode)
        initial_dimension_value = spec.get(
            "initial_dimension_value",
            default_initial_dimension_value(mode),
        )
        ...
        return cls(
            ...
            initial_dimension_value=float(initial_dimension_value),
            dimension_lower_bound=float(spec.get("dimension_lower_bound", lower_bound)),
            dimension_upper_bound=float(spec.get("dimension_upper_bound", upper_bound)),
            tokens=tokens,
            token_graph=graph,
