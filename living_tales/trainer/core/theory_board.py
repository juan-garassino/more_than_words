...
class TheoryBoard:
    ...
    def __post_init__(self) -> None:
        lower_bound, upper_bound = default_dimension_bounds(self.mode)
        self.dimension_lower_bound = lower_bound
        self.dimension_upper_bound = upper_bound
        initial = default_initial_dimension_value(self.mode)
        self.convergence_dimensions = np.full(
            self.n_attractor_dims, initial, dtype=np.float32
        )
    ...
    def add_edge(self, conn: Connection) -> bool:
        ...
        if not contradiction:
            ...
            self.convergence_dimensions = update_dimensions(
                ...
                contribution * self.convergence_rate,
                mode=self.mode,
