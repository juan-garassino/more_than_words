"""
Living Tales — Mystery Card Runner
=================================
...
class GameSpec:
    ...
    min_turns: int
    max_turns: int
    initial_dimension_value: float
    dimension_lower_bound: float
    dimension_upper_bound: float
    tokens: List[Token]
...
def load_cartridge(path: str) -> Tuple[GameSpec, Optional[object]]:
    ...
    spec = GameSpec(
        ...
        mode=manifest.get("mode", "converging"),
        ...
        initial_dimension_value=manifest.get("initial_dimension_value", 0.0),
        dimension_lower_bound=manifest.get("dimension_lower_bound", 0.0),
        dimension_upper_bound=manifest.get("dimension_upper_bound", 1.0),
        tokens=tokens,
...
def action_connect(state: GameState, indices: List[int], model) -> str:
    ...
    if scale > 0:
        ...
        state.casebook.apply_delta(delta)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Living Tales mystery runner")
