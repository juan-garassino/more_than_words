...
from living_tales.trainer.core.creature_case import (
    CREATURE_M_REQUIRED_ROLES,
    classify_creature_token_role,
    creature_role_counts,
)
...
def check(path: Path):
    ...
    if mode == 'oscillating':
        role_counts = creature_role_counts(t['id'] for t in tokens)

        name = 'OSCILLATION required roles'
        missing_roles = sorted(role for role in CREATURE_M_REQUIRED_ROLES if role_counts.get(role, 0) == 0)
        if missing_roles:
            fail(name, f"missing roles: {missing_roles}")

        name = 'OSCILLATION recovery positive'
        bad = [
            t['id']
            for t in tokens
            if classify_creature_token_role(t['id']) == 'recovery'
            and max(t['attractor_weights']) <= 0.0
        ]
        if bad:
            fail(name, f"recovery tokens without positive lift: {bad[:10]}")

        name = 'OSCILLATION decay negative'
        bad = [
            t['id']
            for t in tokens
            if classify_creature_token_role(t['id']) == 'decay'
            and min(t['attractor_weights']) >= 0.0
        ]
        if bad:
            fail(name, f"decay tokens without negative pull: {bad[:10]}")

        name = 'OSCILLATION combo tradeoff'
        bad = []
        for t in tokens:
            if classify_creature_token_role(t['id']) != 'combo':
                continue
            weights = t['attractor_weights']
            non_zero = [v for v in weights if v != 0.0]
            if not non_zero:
                bad.append(t['id'])
                continue
            if not (
                (any(v > 0.0 for v in weights) and any(v < 0.0 for v in weights))
                or len(non_zero) >= 2
            ):
                bad.append(t['id'])
        if bad:
            fail(name, f"combo tokens without mixed-sign tradeoff: {bad[:10]}")
...
def main(argv):
    if len(argv) < 2:
        print("Usage: living_tales_case_validator.py <case.json> [case.json ...]")
        sys.exit(1)
